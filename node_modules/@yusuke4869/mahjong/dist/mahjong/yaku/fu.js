...
const fu = (data, bakaze, jikaze, rempuhai = 2) => {
    // 七対子は一律25符
    if (data.agari.elements.length === 7)
        return 25;
    // 平和形
    const pinfuRes = (0, pinfu_1.checkPinfu)(data, bakaze, jikaze);
    if (pinfuRes.exists) {
        // 喰い平和は一律30符
        if (pinfuRes.furo)
            return 30;
    ...
    for (const element of data.agari.elements) {
        ...
        if (element.type === "toitsu") {
            ...
            if (h === "5z" || h === "6z" || h === "7z")
                sum += 2;
            if (h === bakaze && h === jikaze)
                sum += rempuhai;
            else if (h === bakaze || h === jikaze)
                sum += 2;
            continue;
        }
        // 明刻
    ...
    if (w.penchan || w.kanchan || w.tanki)
        sum += 2;
    // 切り上げ
