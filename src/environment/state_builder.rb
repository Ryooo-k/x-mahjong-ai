...
module StateBuilder
  ...
  def build_current_player_states(player)
    hand_codes = to_codes_count(player.hands)
    called_tile_codes = to_code_table_count(player.called_tile_table)
    river_codes = to_normalized_river_codes(player.rivers)
    score = player.score / ALL_SCORE
    shanten = cal_shanten(player.hands) / MAX_SHANTEN_COUNT
    outs = count_outs(player.hands) / MAX_OUTS_COUNT
  ...
  def to_code_table_count(tile_table)
    counter_table = Array.new(MAX_CALL_COUNT) { Array.new(TILE_COUNT, 0) }
    tile_table.each_with_index do |tiles, order|
      tiles.each { |tile| counter_table[order][tile.code] += 1 }
    end
    counter_table
  end
